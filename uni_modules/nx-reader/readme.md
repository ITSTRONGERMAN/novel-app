...
* 1、这是一个阅读器组件，适用于类似小说阅读功能
* 2、这个插件支持APP-VUE、H5、微信小程序
* 3、本插件会根据当前页面的尺寸，计算每页显示多少个文字，并截取对应的文字来显示
* 4、可以调整文字大小，调整后，会根据调整后的文字大小重新进行计算，并显示对应文字内容
* 5、可以设置阅读背景色，用户也可自行进行配置
* 6、支持点击屏幕中间，出现设置界面 
* 7、支持切换上一章节、下一章节
...
| 属性名称 | 类型 | 默认值 | 可选值 | 说明 |
| :----- | :----: | :----: | :----: | :---- |
| getChapterContentApi | Function | - | - | 获取章节内容API；param：chapterInfo（当前章节信息），returns：章节内容字符串，可以是html代码。支持async |
| toolbarMenu | Array<toolbarMenuItem> | - | - | 菜单栏配置，具体见下方toolbarMenuItem参数说明 |

#toolbarMenuItem属性
| 属性名称 | 类型 | 默认值 | 可选值 | 说明 |
| :----- | :----: | :----: | :----: | :---- |
| name | String | - | - | 菜单名称 |
| icon | String | - | - | 图标名称，与uni-icons的type属性一致。[ 详见 ](https://uniapp.dcloud.net.cn/component/uniui/uni-icons.html#icons-props) |
| fontFamily | String | - | - | 自定义图标字族。[ 使用说明详见 ](https://uniapp.dcloud.net.cn/component/uniui/uni-icons.html#通过-fontfamily-自定义图标) |
| unicode | String | - | - | 自定义图标unicode。[ 使用说明详见 ](https://uniapp.dcloud.net.cn/component/uniui/uni-icons.html#通过-fontfamily-自定义图标) |

#event事件
| 事件名称 | 参数 | 说明 |
| :----- | :----: | :---- |
| init-completed | info = { setChapterList }；setChapterList(chapterList, chapterIndex, pIndex): 设置章节列表，chapterList：章节列表，chapterIndex：当前章节index，pIndex：当前章节要展示的段落index | 组件初始化完成事件 |
| page-change | info = { chapterIndex, topPIndexForChapter }；chapterIndex: 当前页面的章节index，topPIndexForChapter：当前页面首个段落在章节所有段落中的index | 页面切换事件 |

#完整示例
```html
<view class="content">
	<nx-reader
		:get-chapter-content-api="getChapterContent"
	...
	methods: {
		...
		getChapterContent(chapterInfo) {
			return new Promise((resolve, reject) => {
				// 模拟网络加载
				setTimeout(() => {
					const contentInfo = chapterContent.find(item => item.id === chapterInfo.id) || {}
					resolve(contentInfo.content || '');
				}, 300)
			})
		}
		/**
		...
		handlePageChange(info) {
			...
			console.log(topPIndexForChapter);
		}
	}
}
```
